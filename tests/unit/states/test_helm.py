...
def test_release_present_new(chart, release, test):
    mock_release_revision_in = {}

    mock_release_revision_out = dict(
        pyhelm3.models.ReleaseRevision(
            _command="nothing",
            app_version="1.19.2",
            chart=chart,
            name="cert-manager",
    ...
    with (
        ...
        patch.dict(
            helm.__salt__,
            {
                "helm.get_chart": MagicMock(return_value=chart),
...
@pytest.mark.parametrize("test", [True, False])
def test_release_present_changed_values(chart, release, test):
    mock_release_revision_in = {
        "app_version": "1.19.2",
        "chart": {
            "name": "cert-manager",
...
@pytest.mark.parametrize("test", [True, False])
def test_release_present_no_changes(chart, test):
    mock_release_revision_in = {
        "app_version": "1.19.2",
        "chart": {
            "name": "cert-manager",
    ...
    with (
        ...
        assert res["result"] is True
        # pylint: disable=use-implicit-booleaness-not-comparison # we want to assert the dict here
        assert res["changes"] == {}
